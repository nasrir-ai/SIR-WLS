...
* Added `lassosir_fit()`, a thin wrapper around the external `LassoSIR`
  package (CRAN; Lin, Zhao & Liu 2019), matching exactly the call the
  author's own scripts use (`LassoSIR(x, y, H, no.dim = ndim, solution.path
  = FALSE, categorical = categorical, nfolds = nfolds, screening =
  screening)`). This completes the set of three methods compared in the
  paper's Tables 1-6 (`lasso_fit()`, `lassosir_fit()`, `wls.sir()`), all
  callable from `sirwls` with a consistent interface. `LassoSIR` is not
  reimplemented — it's someone else's published method with its own CRAN
  package, added as a `Suggests` dependency.
